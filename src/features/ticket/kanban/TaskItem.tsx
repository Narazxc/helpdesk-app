...
// import type { DropResult, Task } from "./KanbanBoard";
...
const TaskItem: React.FC<TaskItemProps> = ({
  ...
  return (
    <div
      ref={ref}
      style={{ opacity }}
      className="relative p-2 bg-white border border-gray-200 task rounded-xl shadow-theme-sm dark:border-gray-800 dark:bg-white/5"
      data-handler-id={handlerId}
    >
      <div className="space-y-3">
        <div>
          <h4 className="mb-3 mr-10 text-sm font-medium text-gray-800 dark:text-white/90">
            {task.title}
          </h4>
          {task.projectDesc && (
          ...
          <div className="flex items-center gap-2.5">
            <span className="flex items-center gap-1 text-xs text-gray-500 cursor-pointer dark:text-gray-400">
              <svg
                className="fill-current"
                width="14"
                height="14"
          ...
          <span
            className={`mt-2.5 inline-flex rounded-full px-2 py-0.5 text-[10px] font-medium ${getCategoryStyles(
              task.category.color
            )}`}
          >
            {task.category.name}
      ...
      <div className="h-5 w-5 absolute top-4 right-4 overflow-hidden rounded-full border-[0.5px] border-gray-200 dark:border-gray-800">
        <img src={task.assignee} alt="user" />
      </div>
    </div>
  );
