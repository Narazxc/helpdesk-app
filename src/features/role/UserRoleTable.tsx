...
function UserRoleTable() {
  ...
  const [sort, setSort] = useState<SortState>({
    key: "roleName", // Fixed: Updated to match OfficeGroup property
    asc: true,
  });
  ...
  const filteredRows: Role[] = useMemo(() => {
  ...
  const paginatedRows: Role[] = filteredRows.slice(
    (page - 1) * perPage,
    page * perPage
  );
  ...
  const sortBy = (key: "roleName" | "description"): void => {
    setSort((prev) => ({
      key,
              ...
              <th className="w-1/2 p-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400">
                ...
                >
                  ...
                  <span className="flex flex-col gap-0.5">
                    <svg
                      className={
                        sort.key === "description" && sort.asc // Fixed: Updated to description
                          ? "text-gray-800 dark:text-gray-400"
                          : "text-gray-300"
                      }
                      width="8"
